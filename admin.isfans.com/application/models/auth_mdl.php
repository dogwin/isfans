...
class Auth_mdl extends CI_Model{
	...
	function dogwin_get_cookie(){
		...
		if(strlen($C_loginfo)>10){
			...
			$sql = "select * from isfans_administrator where email=? and password=?";
	...
	function user_update($array_data,$email,$password){
		return $this->db->update('isfans_administrator',$array_data,array('email'=>$email,'password'=>$password));
	}
	/**
	 * user login
	 */
	function user_login($email,$password){
		$sql = "select * from isfans_administrator where email=?";
	...
	function check_login($email,$password){
		$sql = "select * from isfans_administrator where email='$email' and password='$password'";
