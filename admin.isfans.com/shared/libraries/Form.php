...
class Form 
{
	...
	private function _content($field, $default)
	{
		...
		$CI = & get_instance();
		if ( ! $CI->platform->cache_exists(HC_JY_SHARE_PATH . 'settings/model/'.$options[0].'.php'))
		{
			return '内存模型不存在!';
		}
	...
	private function _linked_menu($field , $default)
	{
		...
		$ci = & get_instance();
		if ( ! $ci->platform->cache_exists(HC_JY_SHARE_PATH . 'settings/category/data_'.$options[0].'.php'))
		{
			return '分类模型数据不存在!';
		}
	...
	private function _get_data_from_model( & $field , $need_level = FALSE)
	{
		...
		$ci = & get_instance();
		if ( ! $ci->platform->cache_exists(HC_JY_SHARE_PATH . 'settings/category/data_' . $options[0] . EXT))
		{
			return FALSE;
		}
		$ci->settings->load('category/data_' . $options[0]);
